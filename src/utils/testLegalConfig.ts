...
export function testLegalConfiguration(): boolean {
  ...
  const criticalInfo = [
    ...
    {
      name: 'Email contact',
      value: LEGAL_CONFIG.contact.general,
      valid: /^[^@]+@[^@]+\.[^@]+$/,
    },
  ...
  if (security) {
    console.log(
      ...
      security.dataEncryption?.inTransit || 'Non spécifié'
    )
    console.log(
      '✅ Chiffrement au repos:',
      security.dataEncryption?.atRest || 'Non spécifié'
    )
    console.log(
      "✅ Contrôle d'accès:",
      security.accessControl?.principle || 'Non spécifié'
    )
  } else {
...
export function testDocumentPlaceholders(
  ...
  if (remainingPlaceholders && remainingPlaceholders.length > 0) {
    console.log(
      `❌ ${remainingPlaceholders.length} placeholder(s) restant(s) dans ${documentName}:`
    )
...
export function testRGPDCompliance(): ComplianceResult {
  ...
  try {
    ...
    const compliantTransfers = transfers.filter(
      t => t.legalBasis && t.safeguards && t.safeguards.length > 0
    )
