...
export function useCheckout() {
  ...
  const handleCheckout = useCallback(
    async (options?: CheckoutOptions) => {
      ...
      const priceId =
        options?.priceId || process.env.NEXT_PUBLIC_STRIPE_PRICE_ID

      // Validation priceId
      if (!priceId || !/^price_[a-zA-Z0-9]+$/.test(priceId)) {
        console.error(
          '[useCheckout] NEXT_PUBLIC_STRIPE_PRICE_ID manquant ou invalide'
        )
        showToast(
          'Configuration Stripe incomplète. Contacte le support.',
          'error'
        )
      ...
      // URLs par défaut
      const successUrl =
        options?.successUrl || `${window.location.origin}/profil`
      ...
      try {
        ...
        const { data, error, aborted } = await withAbortSafe(
          supabase.functions.invoke<CheckoutResponse>(
        ...
        showToast(
          'Réponse inattendue de Stripe. Réessaie ou contacte le support.',
          'error'
        )
      } catch (e) {
        console.error('Erreur checkout:', String((e as Error)?.message ?? e))
        showToast(
          'Impossible de contacter Stripe. Vérifie ta connexion et réessaie.',
          'error'
        )
      } finally {
        checkingOutRef.current = false
