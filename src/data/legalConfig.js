...
export const LEGAL_CONFIG = {
  ...
  transfers: {
    ...
    googleAnalytics: {
      provider: 'Google LLC',
      country: 'États-Unis',
      legalBasis: 'Consentement explicite de l\'utilisateur (Article 49.1.a RGPD)',
      safeguards: [
    ...
    supabase: {
      ...
      legalBasis: 'Intérêt légitime et exécution de contrat (Articles 6.1.f et 6.1.b RGPD)',
      safeguards: [
      ...
      optOutMechanism: 'Impossible (nécessaire pour le fonctionnement du service)'
    }
  },
...
export function replaceLegalPlaceholders(text) {
  ...
  return text
    ...
    .replace(/\{\{EmailDeContactÀCompléter\}\}/g, LEGAL_CONFIG.contact.general)
    .replace(/\{\{EmailDuSupportÀCompléter\}\}/g, LEGAL_CONFIG.contact.support)
    ...
    // Responsable de publication
    .replace(/\{\{ResponsablePublication\}\}/g, LEGAL_CONFIG.publication.responsible)
    .replace(/\{\{NomDuResponsableÀCompléter\}\}/g, LEGAL_CONFIG.publication.responsible)
    .replace(/\{\{EmailDuResponsableÀCompléter\}\}/g, LEGAL_CONFIG.publication.email)

    // Développement
    .replace(/\{\{NomDeLéquipeDéveloppeurÀCompléter\}\}/g, LEGAL_CONFIG.development.team)
    .replace(/\{\{NomDeLéquipeDeMaintenanceÀCompléter\}\}/g, LEGAL_CONFIG.development.maintenance)

    // Médiation
    .replace(/\{\{MédiateurÀPréciser\}\}/g, LEGAL_CONFIG.mediation.name)
    .replace(/\{\{ContactDuMédiateurÀCompléter\}\}/g, LEGAL_CONFIG.mediation.contact)
    .replace(/\{\{LienVersLeMédiateurÀCompléter\}\}/g, LEGAL_CONFIG.mediation.url)
...
export function checkTransfersCompliance() {
  ...
  transfers.forEach(transfer => {
    const isCompliant = transfer.legalBasis && transfer.safeguards && transfer.safeguards.length > 0
