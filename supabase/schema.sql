...
COMMENT ON FUNCTION "public"."accounts_auto_create_first_child_profile"() IS 'Contrat produit § 2.6: Créer automatiquement le premier espace enfant "Espace 1" à la création d''un compte';
...
COMMENT ON FUNCTION "public"."can_write_sequences"() IS 'Feature gate séquençage: retourne TRUE si le compte courant peut créer/modifier/supprimer des séquences (free, subscriber ou admin). Visitor = local-only IndexedDB. SECURITY DEFINER minimal, search_path hardened.';
...
COMMENT ON FUNCTION "public"."hard_reset_timeline_session"("p_timeline_id" "uuid") IS 'Hard Reset : Supprime toutes les validations, remet la session en active_preview et reset le snapshot. Action explicite adulte (Édition) — déverrouille entièrement la composition.';



CREATE OR REPLACE FUNCTION "public"."import_visitor_sequences_batch"("p_sequences_json" "jsonb") RETURNS "jsonb"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public', 'pg_temp'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."import_visitor_sequences_batch"("p_sequences_json" "jsonb") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."import_visitor_sequences_batch"("p_sequences_json" "jsonb") IS 'Import atomique des séquences Visitor (IndexedDB) vers compte Free/Subscriber.
...
COMMENT ON FUNCTION "public"."reset_active_started_session_for_timeline"("p_timeline_id" "uuid", "p_reason" "text") IS 'Incrémente epoch (Soft Sync) lors d''une modification structurelle de timeline. Appelée par slots_trigger_epoch_increment_after_change. Snapshot immuable — recalcul mid-session supprimé car composition verrouillée pendant active_started.';
...
COMMENT ON FUNCTION "public"."validate_session_state_transition"() IS 'Valide les transitions état session. Autorisé : active_preview→active_started (1ère validation), active_started→active_preview (hard reset adulte), active_started→completed (fin session). Interdit : tout le reste.';
...
CREATE TABLE IF NOT EXISTS "public"."child_profiles" (
    ...
    CONSTRAINT "child_profiles_color_check" CHECK (("color" = ANY (ARRAY['blue'::"text", 'green'::"text", 'orange'::"text", 'purple'::"text", 'red'::"text", 'yellow'::"text", 'teal'::"text", 'pink'::"text"])))
);
...
COMMENT ON COLUMN "public"."child_profiles"."color" IS 'Couleur de l''espace enfant (identifiant visuel). Valeurs: blue, green, orange, purple, red, yellow, teal, pink. Défaut: blue.';
...
COMMENT ON POLICY "sequence_steps_delete_owner" ON "public"."sequence_steps" IS 'Owner via sequence + can_write_sequences() (free/subscriber/admin) + not execution-only: Visitor hors portée (local-only)';
...
COMMENT ON POLICY "sequence_steps_insert_owner" ON "public"."sequence_steps" IS 'Owner via sequence + can_write_sequences() (free/subscriber/admin) + not execution-only: Visitor hors portée (local-only)';
...
COMMENT ON POLICY "sequence_steps_update_owner" ON "public"."sequence_steps" IS 'Owner via sequence + can_write_sequences() (free/subscriber/admin) + not execution-only: Visitor hors portée (local-only)';
...
COMMENT ON POLICY "sequences_delete_owner" ON "public"."sequences" IS 'Owner + can_write_sequences() (free/subscriber/admin) + not execution-only: Visitor hors portée (local-only)';
...
COMMENT ON POLICY "sequences_insert_owner" ON "public"."sequences" IS 'Owner + can_write_sequences() (free/subscriber/admin) + not execution-only: Visitor hors portée (local-only)';
...
COMMENT ON POLICY "sequences_update_owner" ON "public"."sequences" IS 'Owner + can_write_sequences() (free/subscriber/admin) + not execution-only: Visitor hors portée (local-only)';
...
CREATE POLICY "personal_images_avatars_delete_owner" ON "storage"."objects" FOR DELETE TO "authenticated" USING ((("bucket_id" = 'personal-images'::"text") AND ("name" !~~ '%..%'::"text") AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::"text") AND (("storage"."foldername"("name"))[2] = 'avatars'::"text") AND ("split_part"("name", '/'::"text", 3) ~* '^[0-9a-f-]{36}\.jpg$'::"text")));



CREATE POLICY "personal_images_avatars_insert_owner" ON "storage"."objects" FOR INSERT TO "authenticated" WITH CHECK ((("bucket_id" = 'personal-images'::"text") AND ("name" !~~ '%..%'::"text") AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::"text") AND (("storage"."foldername"("name"))[2] = 'avatars'::"text") AND ("split_part"("name", '/'::"text", 3) ~* '^[0-9a-f-]{36}\.jpg$'::"text") AND (EXISTS ( SELECT 1
   FROM "public"."accounts"
  WHERE (("accounts"."id" = "auth"."uid"()) AND ("accounts"."status" = ANY (ARRAY['subscriber'::"public"."account_status", 'admin'::"public"."account_status"])))))));



CREATE POLICY "personal_images_avatars_select_owner" ON "storage"."objects" FOR SELECT TO "authenticated" USING ((("bucket_id" = 'personal-images'::"text") AND ("name" !~~ '%..%'::"text") AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::"text") AND (("storage"."foldername"("name"))[2] = 'avatars'::"text") AND ("split_part"("name", '/'::"text", 3) ~* '^[0-9a-f-]{36}\.jpg$'::"text")));
