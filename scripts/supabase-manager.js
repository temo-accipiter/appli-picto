  ...
  try {
    ...
    try {
      ...
      try {
        ...
        execSync(
          `supabase link --project-ref ${PROJECT_REF} --password-stdin`,
          {
            cwd: projectRoot,
            stdio: 'pipe',
            env: {
        ...
        log(
          'Note : Cette erreur peut être due à Docker. Le projet peut être lié malgré tout.',
          'info'
        )
        ...
        try {
          ...
          if (require('fs').existsSync(configPath)) {
            log(
              'Projet semble être lié (fichier de configuration trouvé)',
              'success'
            )
            return true
          }
  ...
  try {
    ...
    if (tableMatches) {
      const tables = tableMatches.map(match =>
        match.replace('CREATE TABLE public.', '')
      )
    ...
    if (functionMatches) {
      const functions = functionMatches.map(match =>
        match.replace('CREATE FUNCTION public.', '')
      )
    ...
    try {
      const hooks = readFileSync(
        join(projectRoot, 'src', 'hooks', 'index.js'),
        'utf8'
      )
    ...
    try {
      const authContext = readFileSync(
        join(projectRoot, 'src', 'contexts', 'AuthContext.jsx'),
        'utf8'
      )
    ...
    if (tableMatches && tableMatches.length > 0) {
      log(
        `   Base de données : ${tableMatches.length} tables configurées`,
        'success'
      )
    }
    ...
    if (functionMatches && functionMatches.length > 0) {
      log(
        `   Fonctions : ${functionMatches.length} fonctions personnalisées`,
        'success'
      )
    }
    ...
    if (policyMatches && policyMatches.length > 0) {
      log(
        `   Sécurité : ${policyMatches.length} politiques RLS configurées`,
        'success'
      )
    }
...
function updateSchema() {
  ...
  try {
    if (checkSupabaseCLI() && linkProject()) {
      ...
      execSync(
        'supabase db dump --linked --schema public,storage --data-only=false',
        {
          cwd: projectRoot,
          stdio: 'pipe',
        }
...
function setupDocker() {
  ...
  log(
    '3. Reliez le projet : supabase link --project-ref ' + PROJECT_REF,
    'info'
  )
  log('4. Testez la mise à jour : yarn test:supabase:update', 'info')

  log(
    '\n⚠️ Note : Docker est nécessaire pour les opérations CLI avancées',
    'warning'
  )
  log(
    '   Mais votre intégration fonctionne parfaitement sans Docker !',
    'success'
  )
}
...
function compareSchemas() {
  try {
    ...
    if (current === backup) {
      ...
      const currentTableNames = currentTables.map(t =>
        t.replace('CREATE TABLE public.', '')
      )
      const backupTableNames = backupTables.map(t =>
        t.replace('CREATE TABLE public.', '')
      )

      const added = currentTableNames.filter(t => !backupTableNames.includes(t))
      const removed = backupTableNames.filter(
        t => !currentTableNames.includes(t)
      )

      if (added.length > 0)
        log(`Tables ajoutées : ${added.join(', ')}`, 'success')
      if (removed.length > 0)
...
function main() {
  ...
  switch (command) {
    ...
    default:
      log(
        'Usage : node supabase-manager.js [check|update|compare|setup-docker]',
        'info'
      )
